...
public class SumScore {
    ...
    public static void SaveHighScore () {
        if (Score > HighScore) {
            ...
            //GameManager.FindObjectOfType<GameManager>().ReportScore(HighScore);
            if (PlayGamesPlatform.Instance.localUser.authenticated)
            {
                ...
                PlayGamesPlatform.Instance.ReportScore(HighScore,
                    ...
                    (bool success) =>
                    {
                        Debug.Log("(Everville) Leaderboard update success: " + success);
                    });
                if (Score > 10)
                {
                    PlayGamesPlatform.Instance.ReportProgress(
                        GPGSIds.achievement_money_collector,
                        100.0f, (bool successs) => {
                            Debug.Log("(Everville) Money Collector!: " +
                              successs);
                        });
                }
                if (Score > 50)
                {
                    PlayGamesPlatform.Instance.ReportProgress(
                        GPGSIds.achievement_on_a_roll,
                        100.0f, (bool successs) => {
                            Debug.Log("(Everville) On A Roll!: " +
                              successs);
                        });
                }
                if (Score > 100)
                {
                    PlayGamesPlatform.Instance.ReportProgress(
                        GPGSIds.achievement_doing_your_duty,
                        100.0f, (bool successs) => {
                            Debug.Log("(Everville) Doing Your Duty: " +
                              successs);
                        });
                }
                if (Score > 150)
                {
                    PlayGamesPlatform.Instance.ReportProgress(
                        GPGSIds.achievement_show_some_mercy,
                        100.0f, (bool successs) => {
                            Debug.Log("(Everville) Show Some Mercy: " +
                              successs);
                        });
                }
